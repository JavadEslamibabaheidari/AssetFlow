...
M7, Event-Driven Synchronization, is complete, tagged, released, and closed. Its plan is `docs/plans/m7-event-driven-synchronization.md`, its report is `docs/milestone-7-report.md`, and its GitHub tracking is issues #85 through #90 under the closed `M7 - Event-Driven Synchronization` milestone. PRs #91 through #96 are merged, and the final state is tagged and released as `v0.7.0 - M7 Event-Driven Synchronization`.

M7 implements a PostgreSQL-backed outbox instead of direct in-process publication, controller-level publication, or a mandatory Kafka runtime dependency. Application handlers append integration events after successful business mutations. Failed validation, not-found, conflict, oversell, and failed retry paths must not create false downstream success events. The outbox/event contracts are broker-ready so Kafka can be added later without rewriting domain behavior.

The M7 event families are `StockItemCreated`, `StockAvailabilityChanged`, `ReservationCreated`, `ReservationReleased`, `ReservationExpired`, `ChannelSyncRequested`, `ChannelSyncSucceeded`, and `ChannelSyncFailed`. Events use stable envelopes with explicit schema versions, identifiers, occurrence times, payload JSON, processing status, attempt metadata, and safe error details. Stock item creation produces both stock item and initial availability signals; reservation create/release/expire workflows produce reservation lifecycle events and availability signals for affected stock items.

M7 channel synchronization is an idempotent worker path using mockable marketplace adapter interfaces, persisted sync status, explicit retry state, and no live provider credentials. The Operations page evolved from M6 channel master data into backend-backed sync visibility showing no-sync, pending/in-progress, succeeded, failed, and retryable states. Manual retry controls remain absent because M7 implemented automatic retry scheduling but no explicit retry command endpoint.

M7 verification includes event contract tests, application tests for successful and failed mutation paths, outbox persistence/migration coverage, worker idempotency and retry/failure tests, OpenAPI and generated frontend type checks for sync endpoints, Operations page state tests, backend/frontend/integration CI, and Docker Compose configuration validation.
...
M8, Observability and Monitoring, is complete. Its plan is `docs/plans/m8-observability-and-monitoring.md`, its report is `docs/milestone-8-report.md`, and its GitHub tracking is issues #98 through #102 under the closed `M8 - Observability and Monitoring` milestone. PR #103 is merged, and the final state is tagged and released as `v0.8.0 - M8 Observability and Monitoring`.

M8 behavior: keep `/health` as the simple compatibility endpoint and add detailed observability under `/observability/*`. Use dependency-light ASP.NET Core logging, trace/correlation identifiers, built-in .NET `ActivitySource` hooks, JSON operational summaries, and a Prometheus-compatible text endpoint before introducing required observability packages or hosted monitoring infrastructure. Metrics are aggregate and low-cardinality, derived from M7 persisted state in `outbox_messages` and `channel_sync_states`.

M8 adds an optional Docker Compose monitoring profile with Prometheus scraping the API metrics endpoint and Grafana provisioning an AssetFlow overview dashboard. Normal local development does not require monitoring containers. The frontend Operations page summarizes service health, outbox backlog, synchronization failures, retry delay, and trace/correlation hints without trying to replace Grafana as the time-series dashboard.

`Inventory.Api` now has request observability middleware, correlation id propagation through `X-Correlation-ID`, `ActivitySource` tracing around request and channel synchronization work, `/observability/health`, `/observability/metrics`, and `/observability/prometheus`. The Prometheus endpoint emits aggregate `assetflow_` metrics for health, outbox state, event type/status counts, channel sync states, retryable failures, and next retry timestamp. Docker Compose includes an optional `monitoring` profile with Prometheus and Grafana provisioning under `deploy/monitoring/`. The Operations page includes a service observability panel backed by the new JSON health endpoint.

M9, Agentic OS Expansion, is implemented and ready for closure. Its plan is `docs/plans/m9-agentic-os-expansion.md`, its report is `docs/milestone-9-report.md`, and its GitHub tracking is issues #104 through #110 under the open `M9 - Agentic OS Expansion` milestone until the final closure PR is merged and the milestone is tagged/released. M9 expands the repo-local Agentic Control Dashboard into a broader local-first operating layer for memory, repeatable automation, research context, and resource-aware agent workflows.

M9 behavior: the adopted memory layer remains local-first and Git-reviewable through `docs/knowledge/`, `docs/plans/`, `docs/specs/`, milestone reports, and the generated dashboard. M9 explicitly defers Obsidian, vector stores, external drive synchronization, personal notebook sync, and hosted memory infrastructure. `docs/specs/m9-agentic-os-memory-security.md` records the memory decision, data classification, storage rules, access/retention expectations, and review checklist.

M9 automation and resource telemetry are defined in `docs/specs/m9-agentic-os-automation-telemetry.md`. Automation entries are separated into read-only checks, suggested prompts, and approval-required actions. Resource telemetry may use GitHub issues/PRs, local Git metadata, report verification commands, dashboard generation time, and optional sanitized user-provided exports. Provider token/cost totals are unavailable unless explicitly supplied; they must be displayed as unavailable rather than estimated.

M9 external workspace and research integration planning is recorded in `docs/specs/m9-agentic-os-integrations.md`. Google Workspace, calendars, and research notebooks are planned with local-first fallbacks but are not required for the repository. External connector writes, plugin installation, scheduled automations, and sharing/publishing require explicit user approval.
