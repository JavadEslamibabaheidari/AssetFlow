...
M6, Frontend Inventory Parity, is the active planning milestone. Its plan is `docs/plans/m6-frontend-inventory-parity.md`, and its GitHub milestone is `M6 - Frontend Inventory Parity`. The start gate verified that M5 is closed, issue #60 through #66 are closed, PR #73 merged the final M5 validation/docs slice, and tag/release `v0.5.0` exist.

M6 should bring the frontend product surface to the backend state reached at the end of M3. It should cover vendors, products, sales channels, stock items, reservations, and reservation-aware availability, including loading, empty, validation, conflict, not-found, disabled, success, responsive, and containerized frontend/backend verification states. The UI should remain a production operational inventory tool: planning text, milestone status, and task guidance belong in docs and issues, not in the visible app.

M6 frontend workflow direction: keep the M5 app shell, turn Assets, Reservations, and Markets into API-backed work surfaces, make stock item detail the main decision view for on-hand/reserved/available state, and present oversell prevention as normal business feedback. Build the work in issue-scoped slices: UX/API foundation (#74), master-data workflows (#81), stock item and availability workflows (#80), reservation workflows (#79), cross-workflow UX hardening (#78), CI pipeline split (#76), Compose E2E verification (#77), and final validation/docs (#75).
